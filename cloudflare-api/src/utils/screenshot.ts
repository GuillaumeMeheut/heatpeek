...
export async function absolutifyContents(
  html: string,
  baseUrl: string
): Promise<string> {
  const { document } = parseHTML(html);

  absolutifyStylesheets(document, baseUrl);
  absolutifyFonts(document, baseUrl);
  absolutifyImages(document, baseUrl);

  return "<!DOCTYPE html>\n" + document.documentElement.outerHTML;
}

function absolutifyImages(document: Document, baseUrl: string): void {
  const imgTags = Array.from(document.querySelectorAll("img"));
  for (const img of imgTags) {
    const src = img.getAttribute("src");
...
function absolutifyStylesheets(document: Document, baseUrl: string): void {
  const linkElements = Array.from(
    document.querySelectorAll("link[rel='stylesheet']")
  );
  for (const link of linkElements) {
...
function absolutifyFonts(document: Document, baseUrl: string): void {
  const styleElements = Array.from(document.querySelectorAll("style"));
  const fontUrlRegex = /url\(["']?(\/[^"')]+)["']?\)/g;

  for (const styleEl of styleElements) {
    let cssText = styleEl.textContent || "";
    cssText = cssText.replace(fontUrlRegex, (_, path) => {
